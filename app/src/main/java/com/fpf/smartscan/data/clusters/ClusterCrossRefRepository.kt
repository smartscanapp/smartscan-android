...
class ClusterCrossRefRepository(private val dao: ClusterCrossRefDao) {
    ...
    suspend fun getAllCrossRefs(): List<ClusterCrossRefEntity> = dao.getAll()
    suspend fun getByType(mediaType: MediaType): List<ClusterCrossRefEntity> = dao.getByType(mediaType)
    suspend fun getByClusterIds(ids: List<Long>):  List<ClusterCrossRefEntity> = dao.getByClusterIds(ids)
    suspend fun upsertClusterCrossRefs(crossRefs: List<ClusterCrossRefEntity>) {
